# 国内房产投资(房产投资人视角)Master OS — Synthesis

> Phase 2 提炼结果。Phase 3 (skill writeup) 直接消化本文件。
> 行业:国内房产投资 —— 房产投资人 / 投资顾问视角(城市与地段选择 / 政策解读 / 一二手房 / 房产金融与杠杆 / 估值尽调 / 交易税费 / 持有运营 / 退出流动性 / 风险识别 / 资产配置中的房产 / 决策框架与心态)。
> industry_type = `high_capital_practitioner`｜locale = `zh-CN`(中国大陆,聚焦 2021 年以来深度下行期的真实市场)｜保鲜期:工具栈/工作流 3-6 个月,心智模型/智识谱系 1-2 年。
>
> **跨步骤硬规矩遵守说明**:本文所有 `%` / 数字 / 价格 / 日期均带 source_id 或置信度 caveat(统一用「业内估计」标不可精确核验的口径);房价跌幅 / 租金回报率 / 挂牌量等优先挂 gov / 研究院 source_id。
> **时代背景对齐前提(贯穿全文,最高优先级)**:2021 年是分水岭 —— 头部房企爆雷、多地房价下跌、二手房流动性显著变差、烂尾与保交楼是真实问题。本文**绝不沿用「房产永远涨 / 跟我抄底 / 杠杆放大收益 / 闭眼买核心资产」的旧叙事**;凡上行期形成的「涨」向直觉,2024-2025 全部需重新校准。
...
> 9 个维度(复杂行业 7-10 区间),从 7 个心智模型反推。**互斥性自检**:9 个维度产出 9 个不同结论 —— 9.1 判城市基本面 / 9.2 判板块与流动性 / 9.3 查政策外部变动 / 9.4 核产权与标的事实 / 9.5 算完整账与杠杆压力 / 9.6 甄别信源利益结构 / 9.7 判退出时点与止损线 / 9.8 判资产配置占比与自住投资分流 / 9.9 识别红线操作,无 overlap(9.1 城市级 ≠ 9.2 板块/流动性级 ≠ 9.7 退出时点;9.3 查「政策最近改了什么」≠ 9.6 甄别「这个人靠什么赚钱」;9.4 查产权事实 ≠ 9.5 算账;9.8 配置层 ≠ 9.5 单笔算账;9.9 是法律红线/二元判定 ≠ 9.6 利益结构/程度判定)。
...
- **输出格式**:「该城市能级 = {一线/二线/三线};价格方向 = {环比连涨/连跌 N 个月};人口 = {净流入/净流出 + 结构};存销比 = {N 个月,是否超 18-24 个月警戒};购房资格 = {有/无 + 首付几成};结论:{进短名单 / 排除 + 1 句理由}。」
...
- **输出格式**:「目标板块 = {名称};核心驱动 = {产业/交通/学区,哪些有支撑};规划 = {利好已兑现项 + 利空清单};流动性 = {挂牌量水平 + 成交周期 N 个月 + 是否在恶化};资金流向 = {被虹吸/被抽血};结论:{进短名单 / 排除}。」
...
- **输出格式**:「相关政策近 3-6 月变动:{1-3 条具体变化 + 日期 + 全国/本城};政策性质 = {顶层定调/地方落地,松/紧};对该决策的具体影响 = {能不能买/首付几成/能不能卖/月供};建议留的余量 = {1 句,政策反向的预案}。」
...
- **输出格式**:「产权核查结论 = {干净/有瑕疵:具体哪项/不可碰};性质 = {住宅70年 / 商办40年 / 划拨 / 小产权};{一手:开发商爆雷状态 + 预售资金监管是否到位};{法拍:轮候查封/欠费/户口风险};结论:{保留进入估值 / 一票否决}。」
...
- **输出格式**:「净租金回报率 = {X%};盈亏平衡持有年限 = {N 年};三压力场景下现金流 = {能扛/某场景下断};杠杆 = {首付比例 + 是否 100% 自有("100%"= 须全额自有资金的硬规矩口径,业内通行说法) + 现金流可撑 N 个月};结论:{划算 / 不划算 / 杠杆超出可承受范围}。」
...
- **输出格式**:「该信源 = {框架派/自媒体观察样本/自媒体喊单样本/机构数据/反面样本};利益结构 = {靠什么赚钱};这段是 {分析框架 / 荐盘喊单};可信度 = {打几折 + 取框架弃结论 / 零决策权重};结论:{可参考的部分 + 必须扔掉的部分}。」
...
- **输出格式**:「流动性环境 = {能卖/难卖:挂牌量水平 + 成交周期 N 个月};能成交的挂牌价 = {对标网签成交价的区间};限售 = {是否已满锁定期};止损 = {是否触发买入时设的线};结论:{挂牌 / 降价对标成交 / 承认现在卖不掉继续持有 / 按纪律止损}。」
...
- **输出格式**:「房产占家庭资产比例 = {X%,是否过高};可动用流动性 = {够/不够};这套房定性 = {自住/投资/混合 X+Y};应用指标 = {居住效用 / 回报率·流动性·退出};结论:{配置健康 / 占比过高需控制 / 自住投资逻辑要分开评估}。」
...
- **输出格式**:「是否触碰红线 = {否 / 是:具体哪条 R};若是:性质 = {违规/违法} + 后果 = {抽贷/征信污点/法律责任,1 句};对兜售方的判定 = {该中介/顾问不可信,远离};结论:{合规可继续 / 立即停止并拒绝}。」

## 元数据

- **Synthesis date**: 2026-05-14
- **Source counts**: 6 轨 manifest 合计 108 条(figures 27 / tools 18 / canon 12 / sources 18 / glossary 13 / workflows 20);一手 verified_primary + surrogate_primary 约 68 条 ≈ **63%(机械口径)** —— 见 §8.13 关键 caveat:63% 靠 gov + 研究院数据底座撑起,figures 维度机械一手率极低(约 4%),figures 的声音多为严肃媒体转述的二手信号。
- **产出**:7 个心智模型(§1)/ 9 条 playbook(§2)/ 工具栈 4 必备 + 6 场景特化 + 2 新兴 + 9 节点决策树(§3)/ 9 个 workflow 子节 W1-W9(§4)/ 表达 DNA + 4 类 × 9 段对话样本库(§5)/ 4 条质量基准 + 10 条反模式(§6)/ 智识谱系含 5 大流派 + 5 条核心分歧(§7)/ 诚实边界含 intake 全 12 条警示 + 5 条具体局限(§8)/ 9 个 Agentic 维度(§9)。
- **voice_confidence**: **low-medium** —— 对话样本库 9 段全部为 `(转述)`/`(推断)`,无一段 `(原话)`;根因是国内房产 figures 一手内容主阵地(微信公众号/知乎/小宇宙/抖音)全在 zh-CN 信源黑名单上,本 skill 严格不抓取黑名单内容。这是「该行业 + zh-CN 黑名单规则」的结构性产物,已在 §5.2 + §8.13 诚实标注。
- **Cumulative findings issues**: 无阻断性跨 track 矛盾。各 track 的「冷僻信号」均未触发冷僻协议(figures 8 个偏少但是严筛后的诚实数字、canon 偏薄是行业结构性特征、自媒体一手在黑名单是结构性局限且本不应作决策依据)—— 见各节内嵌说明 + §8.13、§8.14。最大的结构性张力是「2024-2025 核心资产长期看好 vs 高增长时代结束」无干净答案,已在 §7.6 保留分歧不裁决。
