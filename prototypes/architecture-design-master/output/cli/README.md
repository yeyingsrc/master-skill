# 现代建筑设计 (从 concept 草图到施工图与现场服务的整套设计流程, 含 BIM / 可持续 / 参数化 / 在地性 模块, 全球语境 + 中国本土实践并重) CLI

把 现代建筑设计 (从 concept 草图到施工图与现场服务的整套设计流程, 含 BIM / 可持续 / 参数化 / 在地性 模块, 全球语境 + 中国本土实践并重) master skill 的认知 OS 物化成 bash 工具。
...
./decision/topic-1.sh
# SOP 走查 (workflow)
./workflow/sop-5-phase-aia-dual.sh
...
| `decision/topic-1.sh` | 设计 决策树 (6 条规则) |
| `decision/topic-2.sh` | 业主 决策树 (3 条规则) |
| `decision/general-playbook.sh` | 通用 Playbook 决策树 (1 条规则) |
| `workflow/sop-5-phase-aia-dual.sh` | 单项目设计 SOP (5 phase, AIA + 中国 dual) SOP 走查 |
| `workflow/sop-5-type.sh` | 事务所类型 SOP (5 type) SOP 走查 |
| `workflow/bim-ai-generative-ca.sh` | BIM + 可持续 + AI generative + 投标 + 业主管理 + CA + 寒冬转型 集成 SOP 走查 |
