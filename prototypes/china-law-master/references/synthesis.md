# 中国法律执业 Master OS — Synthesis (deep)

> 调研日期: 2026-05-03 (deep upgrade). **严肃声明**: 本 skill 给的是资深法律人 (法学者 + 律师 + 法官) 的认知框架, **绝不替代执业律师对个案的法律意见**, 也不是法律咨询. 任何具体争议必须由律师面询 + 完整审查再决定. **中国大陆法语境** (不适用港 / 澳 / 台 / 普通法系).
...
- **输出格式**: 法律状态摘要 + 类案多数倾向 + 争议点 + 不确定性
- **Last_updated**: 2026-05-03 (含民事案件案由 2026-01-01 + 互联网法院新规 2025-11-01)
- **Decay risk**: high — 法律 / 司法解释 / 类案频繁更新, 律师必须持续学习

### 9.2 程序四要素 (管辖 / 时效 / 证据 / 主体)
- **看什么**:
  - **管辖**: 级别 + 地域 + 协议 + 专属 + 涉外
  - **时效**: 一般 3 年 / 短期 1 年 / 最长 20 年 / 商事仲裁不同
  - **证据**: 现有证据 + 待查证据 + 鉴定 / 公证 必要性 + 三性 (真实 / 合法 / 关联)
  - **主体**: 原告 / 被告 / 第三人适格 + 法人 / 个人 / 合伙
- **在哪看**: 民诉法 / 刑诉法 / 行诉法 + 各类司法解释 + 当事人证据 + 调查
- **输出格式**: 四要素逐项评估 + 风险点 + 缺口 (能不能补)
- **Last_updated**: 2023-09-01 (民诉法修正)
- **Decay risk**: low — 程序法相对稳定

### 9.3 实体法分析 (民商事 / 刑事 / 行政)
- **看什么**:
  - **民商事**: 民法典 + 单行法 + 合同 + 类案
  - **刑事**: 刑法 + 修正案 + 司法解释 + 构成要件 + 量刑情节 + 类案
  - **行政**: 行政法 + 行政法规 + 部门规章 + 行政复议 + 行政诉讼
- **在哪看**: flk.npc.gov.cn + 学术教材 (王利明 / 张明楷 等) + 头部所解读
- **输出格式**: 实体法主张 + 抗辩 + 证据匹配 + 法律解释路径 (多选)
- **Last_updated**: 2024-07-01 (公司法 2024 修订) + 2024-03-01 (刑法修正案十二)
- **Decay risk**: medium — 立法 + 司法解释 持续

### 9.4 风险 + 成本 + 可执行性评估
- **看什么**:
  - **胜诉概率** (基于法律状态 + 证据 + 类案)
  - **执行风险** (被告有无可执行财产 / 跨境)
  - **诉讼成本** (诉讼费 + 律师费 + 鉴定费 + 公证费 + 时间)
  - **替代方案** (调解 / 仲裁 / 和解 / 刑事自诉 / 民事附带刑事)
- **在哪看**: 财产调查 + 被告资产 + 执行信息公开网 + 诉讼成本估算
- **输出格式**: 接 / 不接代理决策 + 客户费用预告 + 替代方案推荐
- **Last_updated**: 2025-12 (司法部律师收费标准 / 各地差异)
- **Decay risk**: low

### 9.5 客户期望管理 + 沟通 (软实力)
- **看什么**: 客户具体期望 + 教育水平 + 行业 + 紧迫度 + 既往法律经验 + 情绪状态
- **在哪看**: 接谈 + 持续沟通 + 委托合同 + 风险提示书
- **输出格式**: 期望写进委托合同 + 风险提示书 + 持续节点告知
- **Last_updated**: 2025-12 (律师执业行为规范持续)
- **Decay risk**: low
