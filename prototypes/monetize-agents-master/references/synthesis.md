# Synthesis — monetize-agents (用 AI agent 赚钱)

> Phase 2 蒸馏: 6 轨调研 (1337 行 / 154 manifest URLs / 61% 一手 / 0 黑名单) → 行业认知 OS.
> 学派分歧 = 行业特征不是不确定性. B2B SaaS / Indie hacker / 服务咨询 / VC观察 / 国内 5 派对同一商业问题给完全不同答案, 蒸馏不平均化.

## Intake 摘要 (Phase 0 决策)

- **industry**: 用 AI agent 赚钱 / agent 商业化 (创业 + 副业 + 咨询 + indie hacker)
- **slug**: monetize-agents · **locale**: global (英文圈主导, zh-CN 30% 补充)
- **focus**: operational + business · **profile**: practitioner
- **特殊性**: 公开材料量足但 signal-to-noise 极差 (90% Twitter / YouTube 短视频 / AI hype). 严肃信号集中在长访谈 + VC 长报告 + 创始人 newsletter + Indie Hackers 真实案例.
- **学派**: B2B SaaS enterprise / Indie hacker / 服务咨询 / Framework infra / VC observer / 国内 — 蒸馏保留分歧, 不平均化.
- **法律 / 伦理边界**: API arbitrage / 内容农场 / spam 自动化 等灰色玩法存在但 **不蒸馏** (违反平台 ToS + 用户信任 + 监管风险).
- **时效**: 工具 / 模型价格 / 头部 product 矩阵每 3-6 月可能换. 12 月内必 update.
...
## 3. 工具栈 (集成 Track 02, 蒸到 3 个 cluster)
...
## 9. Agentic Protocol (Phase 3 接口 + 研究协议)

> 拿到一个新 monetize-agents 问题时, agent 先按以下 5 维度做功课, 再用 §1 心智模型 + §2 决策规则给判断. 不只说得像, 更做得像.

**研究维度 (新问题来了先收集这些)**:

1. **学派语境**: 用户在哪派? B2B SaaS / Indie / 咨询 / VC / 国内? 看他用什么术语 (ACV → B2B; MRR + ship → Indie; day rate → 咨询; TAM + thesis → VC; 备案 → 国内). 决定 §2 规则的应用版本
2. **stage 边界**: pre-PMF / post-PMF / scale / exit / pivot 阶段? 不同 stage 适用不同决策 (D1 vs D5 vs D7)
3. **时效新鲜度**: 用户依赖的工具 / 模型价格 / 头部 product 是 12 月内的, 还是更老的? 老的需先 D10 复盘
4. **合规语境**: 大陆 / 港台 / EU / US? 决定哪些合规 obligations 必应 (D8)
5. **学派分歧识别**: 这个问题 5 派会怎么各自答? 揭示分歧根源 (学派身份 / stage / 合规 / 时效) 而非"对错"

→ 5 维度都过了, 再进入 §2 决策规则用对应学派语言输出. 输出**必带学派标签 + 概率边界 + 诚实边界**.

### 9.1 sub_skill 候选 (女娲蒸馏, 选 3 人)

| 人物 | 学派 | 证据厚度 | 长讲材料 | sub_skill_candidate |
|------|------|---------|---------|--------------------|
| **Bret Taylor** | B2B SaaS enterprise | T01-S003 / S004 / S005 / S006 (4 长访谈) | Stratechery 1.5h + Lenny 1h + Cheeky Pint 1h + Sequoia Training Data | **true** (B2B 高 ACV 代言, outcome-based pricing 第一手) |
| **Pieter Levels** | Indie hacker | T01-S015 (Lex 4h+) + levels.io 长帖 + Nomad List 流量数据 | Lex Fridman 4h+ 是单源最密集 founder material | **true** (Indie 极致代表, 1 人 $250K MRR, 反 VC / 反 scale) |
| **Hamel Husain** | 服务咨询 | T01-S025 hamel.dev 全 corpus + Maven AI Evals 课程 + Lenny 长访谈 | 文章 + 课程 + 访谈 三轨 | **true** ("evals are the new code", 咨询派身份代言, 不规模化路径) |
| Harrison Chase | Framework infra | T01-S007 / S008 (Sequoia + LangChain blog) | LangChain blog + Sequoia podcast | **false** (重要但 sub_skill DNA 不如 Bret/Pieter/Hamel 鲜明; 留 §3 工具栈讨论) |
| Olivia Moore | VC observer | T01-S031 a16z agent landscape | a16z blog + podcast | **false** (VC 视角好但 thesis writer 不如 founder 实战 sub_skill 价值大) |

**最终推荐**: **Bret Taylor + Pieter Levels + Hamel Husain** 3 人 — 跨派覆盖 90%+ commercial-model 空间 (B2B 高 ACV / Indie solo / 咨询 expertise), 都有 ≥ 30 min 长访谈, 思维 DNA 区分鲜明, 分别对应 D1-D10 的核心引用源.

### 9.2 cli 候选 (cli_writer.py 抽取)

按主题 cluster 建议:
- `cli/decision/school-fit.sh` — 学派定位决策 (D1 + W1)
- `cli/decision/pmf-validation.sh` — PMF 验证 (D2 + W2)
- `cli/decision/pricing-model.sh` — 定价模型选择 (D3 + W4)
- `cli/decision/gtm-launch.sh` — GTM 启动 (D4 + W3)
- `cli/decision/scale-vs-stay.sh` — Scale 决策 ($10K MRR 分水岭, D5 + W6)
- `cli/decision/compliance-region.sh` — 监管 / 合规 (D8 + W8)
- `cli/protocol/agentic.sh` — 5 研究维度反推 + 心智模型应用 (Phase 2.9)

### 9.3 Cumulative findings (跨 track 矛盾标注)

- **学派语言混用风险**: 同一术语 (e.g. "production", "free", "moat") 在 B2B vs Indie vs 咨询 语境含义不同. 跨派对话先确认语境 (Track 06 §5 派对照矩阵)
- **VC 派 vs Indie 派的 hype 不对称**: a16z / Sequoia 偏 enterprise B2B 视角, Indie 派 (Levels / Vassallo) 持续抗衡 — 蒸馏不偏任一派
- **国内一手 ARR 数据稀薄** (Canon §10 提示) — 国内派蒸馏依赖团队访谈 + 政府备案数据 + 长 podcast (张小珺 / 硬地骇客), 缺 Indie hacker 模式真实复制案例
- **灰色玩法 (API arbitrage / 内容农场)** 在公开材料中比例不小 (营销号占 90%) 但**蒸馏明确剔除** — 这是诚实边界, 不是 Phase 1.5 信号薄弱
...
- **Source counts**: T01 figures 32 (53% primary), T02 tools 27 (46% primary), T03 workflows 13 (62% primary), T04 canon 38 (100% primary), T05 sources 25 (24% primary 但实质≥18 是一手 — verifier whitelist 限制), T06 glossary 19 (≥60% primary). 总 154 manifest URLs / 61.0% 一手率 / 0 黑名单
- **Cumulative findings issues**: 见 §9.3 — 学派分歧 = 行业特征 (B2B vs Indie 互相挑衅, VC vs Indie hype 不对称, 国内 vs 海外法律 / 文化差异)
- **冷僻信号**: 候选心智模型 14+ (≥ 5 floor 达, emerging commercial), playbook 候选 18+ (≥ 8 floor 达). 量足但 signal-to-noise 极差 — 蒸馏靠 Wave 1 严格筛选 (长访谈 / VC 报告 / 创业者博客 retain, hype / 营销号 / 短视频 排除).
- **关键 decay 时点**:
  - **3-6 月**: LLM 价格降一次 / framework 主流换一次 / 平台 distribution 政策变 (X 算法 / ProductHunt / HN)
  - **12 月**: 头部 product 矩阵洗一遍 (a16z 年度 top 100 list 改 30%+) / monetize-agents 学派代表人物可能换
  - **24 月**: 整体行业战略叙事可能重塑 (e.g. agent 是替代 SaaS 还是 SaaS 一层?)
- **本 skill 12 月后必须 update**: monetize-agents 是高 decay 行业, 不 update 等于过期
