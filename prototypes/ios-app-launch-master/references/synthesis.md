# Synthesis — ios-app-launch (iOS 应用上架大师)

> Phase 2 蒸馏: 6 轨调研 (1227 行 / 169 manifest URLs / 70.4% 一手 / 0 黑名单) → 行业认知 OS.
> 本 skill 不蒸馏人物 sub-skill (用户指示 Phase 3.3-3.5 跳过).
> 学派分歧 = 行业特征. Apple 官方 / 海外 indie / 大厂 release engineering / ASO 优化派 / 国内合规派 / 反 Apple 反垄断 6 派对同一上架问题给完全不同答案, 蒸馏不平均化.

## Intake 摘要 (Phase 0 决策)

- **industry**: iOS App Store launch — 审核 + ASO + 合规 + 多区 + 付费策略
- **slug**: ios-app-launch · **locale**: global (海外 Apple 单家 vs 国内 Apple+ICP+算法备案+8-10 应用市场, 双合规体系完全不同)
- **focus**: operational + technical · **profile**: practitioner
- **特殊性**: Apple App Review 是黑盒 (拒绝率 20-40% 业内估计, Apple 不公开). 政策高频变化 (年度 WWDC + 季度 ASC + iOS major 每年 9 月). 12 月以前的攻略多数已过时.
- **学派**: Apple 官方 / 海外 indie / 大厂 release eng / ASO 优化派 / 国内合规派 / 反 Apple 反垄断 — 蒸馏保留分歧.
- **法律 / 伦理边界**: 不蒸 fraudulent 玩法 (作弊审核 / fake review / TestFlight 公链当上架 / 国内版号灰产 / 代过审).
- **时效**: WWDC 6 月 + iOS major 9 月 + ASC 季度 + DMA / Anti-steering 持续演化. 12 月内必 update.
...
> 拿到一个新 ios-app-launch 问题时, agent 先按以下 5 维度做功课, 再用 §1 心智模型 + §2 决策规则给判断. 不只说得像, 更做得像.

**研究维度 (新问题来了先收集这些)**:

1. **学派语境**: 用户在哪派? Apple 派 / Indie / 大厂 release eng / ASO / 反 Apple / 国内合规? 看他用什么术语 (ARG 条款号 → Apple 派; "我又被拒了" → Indie; CPP/A/B → ASO; 30% Tax → 反 Apple; ICP/备案 → 国内). 决定 §2 规则的应用版本
2. **stage 边界**: 0→1 上架 / 已上架持续运营 / scale-up 多区 / 拒审救火 / 政策适配? 不同 stage 适用不同决策 (D1 vs D3 vs D10)
3. **区域语境**: US / EU / CN / 多区? 决定哪些合规 obligations 必应 (D5 + D7 + D8). DMA 后 EU 不同, 大陆完全不同
4. **时效新鲜度**: 用户依赖的政策是 12 月内的? 老的需先 D3 复盘. iOS 26 SDK 2026-04-28 / Privacy Manifest 2024-05-01 / Anti-steering 2024-01 / DMA 2024-03-07 等都是 12 月内强制 deadlines
5. **学派分歧识别**: 这个问题 6 派会怎么各自答? 揭示分歧根源 (Apple 派 vs 反 Apple / Indie audience vs ASO 数据 / 海外 vs 国内) 而非 "对错"

→ 5 维度都过了, 再进入 §2 决策规则用对应学派语言输出. 输出**必带学派标签 + 区域语境 + 诚实边界**.

### 9.1 sub_skill 候选 (跳过, 用户指示)

**用户明确指示不蒸馏人物 sub-skill** (Phase 3.3-3.5 跳过). Track 01 figures (16 figures) 仍调研为 行业认知地图, 但不实例化为可装载的 person sub-skill.

注: 如果未来需要 sub-skill, 跨派候选建议 (供未来重启使用):
- Apple 派: Phil Schiller (Apple Fellow, App Store director) — 法庭文档 + 偶尔 podcast
- Indie 派: Marco Arment (Overcast, ATP) — marco.org 15+ 年长博客 + ATP weekly podcast
- 反 Apple 派: DHH (Basecamp / Hey) — hey.com/apple/ 公开战 + Twitter long threads
- ASO 派: Eric Seufert (Mobile Dev Memo) — newsletter + 商业 + ASO 视角
- 国内合规派: 曹道富 (鸟哥笔记) — niaogebiji.com + 长文 corpus

### 9.2 cli 候选 (cli_writer.py 抽取)

按主题 cluster 建议:
- `cli/decision/rejection-handling.sh` — Rejection 救火决策 (D2 + W2)
- `cli/decision/aso-vs-audience.sh` — ASO 数据驱动 vs Indie audience-first 选择 (D4 + W3)
- `cli/decision/region-strategy.sh` — 多区上架决策 (D5 + W6)
- `cli/decision/monetization-model.sh` — 商业模型 / 30% Tax 应对 (D6 + W7)
- `cli/decision/privacy-compliance.sh` — 隐私三件套 (D7 + W4)
- `cli/decision/cn-compliance.sh` — 国内 4 件套 (D8 + W5) — **关键, 国内出海必读**
- `cli/decision/policy-tracking.sh` — 政策时效跟踪 (D10 + W8)
- `cli/protocol/agentic.sh` — 5 研究维度反推 + 心智模型应用 (Phase 2.9)

### 9.3 Cumulative findings (跨 track 矛盾标注)

- **学派语言混用风险**: 同一术语 (e.g. "review", "production", "free") 在 Apple 派 vs Indie 派 vs ASO 派语境含义不同. 跨派对话先确认语境 (Track 06 §学派对照矩阵)
...
- **Source counts**: T01 figures 28 (50% primary), T02 tools 29 (72% primary), T03 workflows 21 (76% primary), T04 canon 35 (71% primary), T05 sources 32 (81% primary), T06 glossary 24 (~67% primary). 总 169 manifest URLs / 70.4% 一手率 / 0 黑名单
- **Cumulative findings issues**: 见 §9.3 — 学派分歧 = 行业特征, 30% Tax 立场不对称, 国内一手长访谈稀薄, Apple 特殊渠道 indie 不可用
