# 求职 / 面试辅导（求职教练视角）Master OS — Synthesis

> Phase 2 提炼结果。Phase 3（skill writeup）直接消化本文件。
> industry_type=`coaching_practitioner`，locale=`global`（中国大陆 + 海外英语区，两套体系差异极大），profile=`practitioner`。
...
> 9 个维度，从 §1 心智模型反推「这个模型让人首先关注什么」。每个维度三段「看什么 / 在哪看 / 输出格式」。维度互斥 —— 已自检 9 个维度能独立产出不同结论。
...
- **输出格式**：一句话锁定 locale + 走哪条决策树（A 海外 / B 国内 / 跨两套），并标注「跨 locale 的工具与经验几乎不可移植」。
...
- **输出格式**：判定「定位是否清晰」—— 不清晰则标「先做 W5 定位收敛，目标不收敛不进简历 / 投递」；清晰则直接进卖点提炼 + 故事线。附约束条件清单。
...
- **输出格式**：定位卡点在哪一层 + 该层的根因假设 + 优先解决项（举例性质，非固定阈值：如「投递→初筛转化率明显偏低，根因高频在定位或简历，优先排查 W5/W6」）。
...
- **输出格式**：一份「岗位痛点 + 关键词 + 面试流程 + 招聘方 AI 使用情况」摘要，作为简历组织和面试准备的原点。
...
- **输出格式**：判定主路径（networking 优先 / 网申优先 / 两者并行）+ 据此排工具优先级（networking 路径下 ATS 优化降级、tracker 升级反之）。
...
- **输出格式**：一句话市场周期判断（带日期 + 官方源）+ 结构性约束清单（标「真实存在但程度因行业 / 岗位差异，可用经历 / 作品 / 内推 / 定位对冲」）+ 明确「可控 vs 不可控」边界，不 victim-blame。
...
- **输出格式**：一个有数据支撑的 anchor + walk-away 线 + 可用的真实杠杆清单 + 该地区「先报数字」战术是否需变形。明确「编造 offer 是红线」。
...
- **输出格式**：标记出具体的营销噪音 / 黑产话术 + 为什么行业拒绝它 + 内行的真实说法（如「保 offer = 兜售确定性，教练只能提升概率」）；标记利益冲突;标记红线行为并明确后果（背调暴露 / 试用期崩盘 / 职业污点）。
...
- **输出格式**：标记方案中「时效敏感、需自行核实」的部分 + 给出「原理」替代「当下话术」（如「不给具体 prompt 模板，给『ATS 是解析 + 检索、AI 味要去掉』的原理」）+ 建议 update 频率。

## 元数据

- **Synthesis date**: 2026-05-14
- **Source counts**: 6 轨 manifest 合计 163 条 source —— figures 36 / tools 34 / canon 20 / sources 25 / glossary 25 / workflows 23。一手（verified_primary + surrogate_primary）约 77 条 ≈ 47%（机械口径；creator 裸域博客 / Substack / YouTube 频道、招聘平台 vendor docs 被 source_verifier 保守判 secondary 所致，作者实质一手内容比例远高于机械数字 —— figures track 机械 8/36 ≈ 22% 但实质口径远高）。
- **提炼产出**：心智模型 7 个（§1）/ 标准 playbook 9 条（§2）/ 工具栈三层 5+6+3 + 9 节点决策树（§3）/ workflow 子节 9 个 W1-W9（§4）/ 表达 DNA 5 节 + 对话样本库 4 类共 16 段 voice_confidence=medium（§5）/ 质量基准 4 条 + 反模式 10 条（§6）/ 智识谱系 5 大路线分歧（§7）/ 诚实边界 intake 10 条警示全消化 + 一手率口径 + 国内信号薄 + 5 条具体局限（§8）/ Agentic Protocol 9 个研究维度（§9）。
- **Cumulative findings issues（未解决的跨 track 矛盾）**：无实质性跨 track 数据矛盾。各 track 高度一致，且共同指向同一组结构性诚实边界 —— (1) 国内 vs 海外不对称（海外 figure / 信息源 / canon 密度显著高于国内，国内一手主阵地在黑名单平台）；(2) 一手率机械口径偏低是 source_verifier 对求职 creator 裸域内容的机械局限，非调研缺陷；(3) coaching_practitioner 行业无「production case」式工具 / 流程证据，多为「行业现状观察」。这三条已在 §8 如实呈现，Phase 2.5 review 时无需用户裁决矛盾，但需用户确认「国内外不对称如实呈现、不拉平」的处理方式。
- **行业基调提示（传递给 Phase 3）**：这个领域营销噪音 +「包 offer」黑产极重，intake 明确要求「不要为了 SKILL.md 好看而软化警示」。§8 的 10 条警示、§7.4 的机构模式批评、§5.X.4 的反例版话术、§6.2 的 victim-blaming 反模式 —— 都必须保持当前的硬度，Phase 3 写 SKILL.md 时不软化。资深求职教练的标志恰恰是「不承诺确定性」。
