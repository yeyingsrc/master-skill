# 软件架构 Master OS — Synthesis

> Phase 2 提炼. Phase 3 skill_writer 直接消化本文件. Industry: software architecture (application & system architecture, **不含** hardware / 纯 enterprise architecture / 纯 DevOps / 纯 security), locale=global (中外双语并重, 现实比例 全球 ~75% + 中国 ~25% (本 prototype metadata)), profile=practitioner, focus={primary: operational, secondary: academic}.
>
> Synthesis 输入: 6 轨 research notes, 总 544 manifest rows (verified_primary 43 + surrogate_primary 462 + secondary 23 + reference 15 + dead 1), 一手率 **92.8% (本 prototype metadata; Phase 1.5 修后 — 历史新高, 超过 china-law 89% 业内 baseline)**, 0 blacklisted.
...
## 9. Agentic Protocol — 7 个研究维度

> 拿到一个新软件架构问题时, 本 OS 默认按以下 7 维度做功课再答, 而不是直接给 "best practice".

### 9.1 场景上下文识别 (context discovery)

- 公司规模 (eng 数 / service 数): 5-15 创业 / 50-200 scale-up / 1000+ 大厂 / enterprise 重 governance — **决定 80% (业内估) 解法**
- 业务 vertical: web / mobile / data-intensive / LLM-native / 银行 / 电商 / 视频流
- 团队结构 (Team Topologies): stream-aligned / platform / enabling / complicated-subsystem
- 数据规模 (rows / TB / events per sec)
- 当前栈 (legacy 程度 / cloud provider / 主语言 / 主 db)

### 9.2 NFR 与硬约束 (non-functional requirements)

- SLO / SLI / error budget (latency p99 / availability target / throughput)
- RPO / RTO (灾难恢复)
- 一致性需求 (strong / eventual / tunable)
- 合规 (GDPR / SOX / 等保 / 国密 / 信创)
- 预算 (cloud bill ceiling / team headcount / time to market)
...
- 自建 vs SaaS (build vs buy)

### 9.4 历史与对手案例 (industry case lookup)

- 类似规模公司怎么解决类似问题 (Stack Overflow / Shopify / DHH / Netflix / 阿里 / 字节)
- 大厂 engineering blog (netflixtechblog.com / shopify.engineering / stripe.com/blog) 一手 case
- 失败案例 (反例) 同样重要 — Knight Capital 2012 / Heroku / Mt.Gox 等

### 9.5 演进路径 + fitness function

- 不画终局图, 画 3 个 milestone 的演进图 (今天 / 6 月 / 18 月)
- 每个 milestone 配 fitness function (自动化测试守护"什么算好")
- strangler fig pattern / branch-by-abstraction / feature flag / canary release 渐进策略

### 9.6 AI 辅助边界 (2024-2026 当前 vertical)

- 哪段 LLM 帮 (boilerplate / ADR draft / pattern lookup / test gen / commit message / code review 辅助)
- 哪段 LLM 不帮 (service boundary / polyglot persistence / 跨系统 trade-off / business ↔ tech 翻译 / 新颖问题首发)
- LLM-native 应用架构本身 (RAG / agent loop / eval / cost-aware routing) 单算一类

### 9.7 ADR 闭环 (decision record + review)

- 决策记录 (lightweight ADR, ≤ 1 页): what / why / alternatives / consequences / exit strategy
- 评审节奏 (per-PR / 月度 / 季度)
- 退出策略 (vendor lock-in / 架构债 / 技术债 退出路径)
- ADR 存 `docs/adr/` git-versioned, 跟代码同生命周期

---

## 10. Synthesis 自查

- §1 心智模型: 7 个 ✓ (1.1-1.7), 每个含一句话 / 应用 / 局限 / evidence ≥ 4 source_ids
- §2 playbook: 10 条决策启发式 ✓ + 元规则 (何时违背)
- §3 工具栈: 必备 6 + 场景特化 **8 类** ✓ (header 显式数字) + 新兴 6 + 避坑 7
- §4 工作流: 入门 SOP 8 phase + 资深路径 + 近期变化 + 7 个横向场景 ✓
- §5 voice 库: 4 类 × 2-3 段 = 10 段 ✓ (5.1 客户 2 段 + 5.2 同业 3 段 + 5.3 学术 3 段 + 5.4 反例 4 段); 推断率: 5.1.2 paraphrase / 5.3.3 paraphrase / 5.4.3 paraphrase = 3/10 = 30% (本 skill metadata, 在 ≤ 30% 红线内)
- §6 质量基准 5 条 + 反模式 10 条 ✓
- §7 智识谱系 10 流派 ✓ + anti-figures 2 个 + sub-skills top 3
- §8 诚实边界 9 条 ✓ 全 explicit
- §9 Agentic Protocol 7 维度 ✓
